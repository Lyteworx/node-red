...
settings.getListenPath = function() {
    var listenPath = 'http'+(this.https?'s':'')+'://'+
                    (this.uiHost == '0.0.0.0'?'127.0.0.1':this.uiHost)+
                    ':'+this.uiPort;
    if (this.httpAdminRoot !== false) {
        listenPath += this.httpAdminRoot;
    } else if (this.httpStatic) {
        listenPath += "/";
    }
    return listenPath;
};
